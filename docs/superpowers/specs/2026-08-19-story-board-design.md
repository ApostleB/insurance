...
- 기본 5초마다 다음 글로 자동 전환
- **순환**: 마지막 글 다음은 처음 글 (`index = (index + 1) % slides.length`), 역방향도 동일
- 슬라이드 클릭 시 `/story/:id` 상세로 이동
- **데스크톱**: 마우스 오버 → 정지 / 마우스 아웃 → 재개
- **모바일**: 터치 시작 → 정지 / 터치 종료 후 잠깐 뒤 재개
- **모바일 스와이프**: 좌우 스와이프로 다음/이전 글 수동 이동
...
- 입력: 제목, 대표 이미지(파일 첨부), 원문 링크(선택), Quill 본문
